...
// ggml_compute_forward_kvarn_varn
//
// KVARN VarN (item 04): SINQ log-domain std-dev variance normalization, ported from
// the llama-layer reference (src/llama-kvarn.cpp). ggml is a lower layer than llama
// and cannot include its headers, so the math is duplicated here; the GPU port lives
// in ggml-cuda/kvarn-varn.cu. Any change must stay in lockstep across all three.

static const double KVARN_VARN_VAR_EPS = 1e-12;

static void kvarn_varn_normalize_columns(float * T, float * L_c, int R, int C, float c_min, float c_max) {
    double tile_mean = 0.0;
    for (int k = 0; k < R * C; k++) tile_mean += (double)T[k];
    tile_mean /= (double)(R * C);
    for (int j = 0; j < C; j++) {
        double mean = 0.0;
        for (int i = 0; i < R; i++) mean += (double)T[i * C + j];
        mean /= (double)R;
        double m2 = 0.0;
        for (int i = 0; i < R; i++) { double d = (double)T[i * C + j] - mean; m2 += d * d; }
        double v = m2 / (double)R;
        if (v <= KVARN_VARN_VAR_EPS) {
            double dev = mean - tile_mean;
            v = dev * dev;
            if (v <= KVARN_VARN_VAR_EPS) continue;
        }
        double delta = 0.5 * std::log(v);
        float  L_new = std::clamp((float)((double)L_c[j] + delta), c_min, c_max);
        float  f     = std::exp(L_new - L_c[j]);
        for (int i = 0; i < R; i++) T[i * C + j] /= f;
        L_c[j] = L_new;
    }
}

static void kvarn_varn_normalize_rows(float * T, float * L_r, int R, int C, float c_min, float c_max) {
    double tile_mean = 0.0;
    for (int k = 0; k < R * C; k++) tile_mean += (double)T[k];
    tile_mean /= (double)(R * C);
    for (int i = 0; i < R; i++) {
        double mean = 0.0;
        for (int j = 0; j < C; j++) mean += (double)T[i * C + j];
        mean /= (double)C;
        double m2 = 0.0;
        for (int j = 0; j < C; j++) { double d = (double)T[i * C + j] - mean; m2 += d * d; }
        ...
        float  f     = std::exp(L_new - L_r[i]);
        for (int j = 0; j < C; j++) T[i * C + j] /= f;
        L_r[i] = L_new;
    }
}

static float kvarn_varn_imb_metric(const float * tile, int R, int C) {
...
static void kvarn_varn_normalize_tile(float * T, int R, int C, int K, float c_min, float c_max, float * S_c, float * S_r) {
    if (R <= 0 || C <= 0 || K <= 0) {
    ...
    for (int iter = 0; iter < K; iter++) {
        kvarn_varn_normalize_columns(T, L_c.data(), R, C, c_min, c_max);
        kvarn_varn_normalize_rows(T, L_r.data(), R, C, c_min, c_max);
        float curr_Imb = kvarn_varn_imb_metric(T, R, C);
        if (curr_Imb < best_Imb) {
            best_Imb = curr_Imb;
            memcpy(best_T.data(),   T,          (size_t)R * C * sizeof(float));
            memcpy(best_L_c.data(), L_c.data(), (size_t)C     * sizeof(float));
            memcpy(best_L_r.data(), L_r.data(), (size_t)R     * sizeof(float));
        }
    }
    memcpy(T, best_T.data(), (size_t)R * C * sizeof(float));
    for (int j = 0; j < C; j++) S_c[j] = std::exp(best_L_c[j]);
    for (int i = 0; i < R; i++) S_r[i] = std::exp(best_L_r[i]);
}

void ggml_compute_forward_kvarn_varn(
    const struct ggml_compute_params * params,
          struct ggml_tensor * dst) {
    ...
    float * out = (float *) dst->data; // [T_norm(nt) ++ S_r(n_ch) ++ S_c(n_head*n_tok)]

    memcpy(out, Tin, nt * sizeof(float));
    float * S_r = out + nt;
    float * S_c = S_r + n_ch;
    for (int h = 0; h < n_head; h++) {
        float * Th = out + (size_t) h * head_dim * n_tok; // [head_dim, n_tok] sub-tile, in place
        kvarn_varn_normalize_tile(Th, head_dim, n_tok, 12, -5.0f, 5.0f,
                                  S_c + (size_t) h * n_tok,
